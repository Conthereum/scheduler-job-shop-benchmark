...
def conthereum_priority(simulationEnv, operation):
    """ ConThereum Rule: The Least conflicts """
    # Todo - FIFO at the time
    return len(operation.job._conflicting_jobs);
...
def conthereum_rule():
    """ SPT Rule: Shortest Processing Time - but all the operations of a job on machines are equal """
    return True
